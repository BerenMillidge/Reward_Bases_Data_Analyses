# these scripts create the figures in Figure 2
import numpy as np
...
# code for value learning simulation
def run_simulation(N_runs = 200, lr = 0.1, N_seeds = 10, regression_type = "reward_type_only", double_update = False,use_subjective_value_reward = True):
    ...
    plt.legend(loc="upper right", fontsize=26, alpha=0.5)
    plt.xticks(fontsize=28)
    plt.yticks(fontsize=28)
    plt.tight_layout()
    fig.savefig("value_function_learning_" + str(regression_type) + ".png", format="png")
    plt.show()

# plot the coefficient scatter plot in fig2
def plot_coefficient_scatter(regression_type = "reward_type_only"):  
    ...
    plt.xlabel("Juice coefficient",fontsize=28)
    plt.ylabel("Food coefficient",fontsize=28)
    plt.ylim(-0.5,2)
    plt.xlim(-0.5,2)
...
def plot_simulated_neuron_timecourse(regression_type="reward_type_only", no_xticks = True):
    ...
    for i in range(5):
        print("IDX: ", i)
        delta_min = (food_coeffs[min_juice_idx] * (rs_food[i] - avg_food)) + (juice_coeffs[min_juice_idx] * (rs_water[i] - avg_water))
        predicted_vals_min[i,2] = delta_min
        print("delta min: ", delta_min)
        delta_max = (food_coeffs[max_juice_idx] * (rs_food[i] - avg_food)) + (juice_coeffs[max_juice_idx] * (rs_water[i] - avg_water))
    ...
    plt.title("Simulated food neuron",fontsize=FONT_SIZE)
    plt.tight_layout()
    fig.savefig("simulated_food_neuron_" + str(regression_type) + ".png", format="png")
    plt.show()
    ...
    plt.tight_layout()
    fig.savefig("simulated_juice_neuron_" + str(regression_type) + ".png", format="png")
    plt.show()
